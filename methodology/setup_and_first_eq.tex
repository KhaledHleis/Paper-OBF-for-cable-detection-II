...
Starting from the expression of the magnetic field in the CPA frame of reference, and noting that, as a telecommunications cable, the return current is via the seawater, which motivates modeling the cable as an infinite straight conductor carrying current in a single direction, under this assumption, the magnetic field is obtained using the classical Biot–Savart law:
...
For simplicity, we assume that the sensor does not rotate; i.e., its local \(x\) and \(z\) axes remain aligned with those of the CPA frame at all times.
